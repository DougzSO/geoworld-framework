...
[docstring mantida — omitida por brevidade]
"""
...
try:
    ...
    _TRANSPORT_PLOTS_AVAILABLE = True
except ImportError:
    _TRANSPORT_PLOTS_AVAILABLE = False
    plot_emissions_trajectory = plot_fleet_transition = None
    plot_renewable_need = plot_hub_map = None
...
_PT_ENERGY_KEY: Dict[str, str] = {k: v for k, v in _PT_EF_KEY.items()}
...
def _interpolate_trajectory(
    anchors: Dict[int, float], years: List[int]
) -> Dict[int, float]:
...
def _interpolate_powertrain_shares(
    ...
    for k, v in scenario_anchors.items():
        ...
        if isinstance(v, dict):
            int_anchors[yr] = {
                pt: float(v.get(pt, 0.0)) for pt in _POWERTRAIN_TYPES
            }

    if not int_anchors:
        return {
            y: {"ice": 1.0, "hev": 0.0, "phev": 0.0, "bev": 0.0, "fcev": 0.0}
            for y in years
        }
    ...
    for y in years:
        ...
        if total > 0:
            ...
            result[y] = {
                "ice": 1.0, "hev": 0.0, "phev": 0.0, "bev": 0.0, "fcev": 0.0
            }
...
# ═══════════════════════════════════════════════════════════════════════════
# PotentialView — typed adapter over PotentialResult or plain dict
# ═══════════════════════════════════════════════════════════════════════════

class _PotentialView:
    """
    Uniform accessor for Phase 4 results regardless of format.

    Accepts either:
      - ``PotentialResult`` Pydantic model  (normal pipeline path)
      - Plain ``dict``                      (skip-mode recovery path)
      - ``None``                            (Phase 4 skipped entirely)

    Exposes ``capacity_gw(tech, scenario)`` so callers never do
    ``result["techs"][tech]["scenarios"][sc]["capacity_gw"]`` inline.
    """

    def __init__(self, pot_results: Any) -> None:
        self._raw = pot_results

    def capacity_gw(self, tech: str, scenario: str = "balanced") -> float:
        """Return installable capacity (GW) for tech/scenario, 0.0 on miss."""
        if self._raw is None:
            return 0.0
        try:
            # Pydantic model path
            if hasattr(self._raw, "techs"):
                tech_obj = self._raw.techs.get(tech)
                if tech_obj is None:
                    return 0.0
                # PotentialResult.techs[tech].scenarios[sc].capacity_gw
    ...
    def as_re_dict(self) -> Dict[str, float]:
        """Return {solar, wind, biomass} capacity (GW) for balanced scenario."""
        return {
            tech: self.capacity_gw(tech, "balanced")
...
# ═══════════════════════════════════════════════════════════════════════════
# LCOEView — typed adapter over LCOEResult or plain dict
# ═══════════════════════════════════════════════════════════════════════════

class _LCOEView:
    """
    Uniform accessor for Phase 5 results regardless of format.

    Accepts either:
      - ``LCOEResult`` Pydantic model  (normal pipeline path)
      - Plain ``dict``                 (skip-mode recovery path)
      - ``None``                       (Phase 5 skipped)

    Exposes ``mean_lcoe(tech)`` and ``supply_curve_max_gw(tech)``.
    """

    def __init__(self, lcoe_results: Any) -> None:
        self._raw = lcoe_results

    def mean_lcoe(self, tech: str, fallback: float = 60.0) -> float:
        """Return spatial-mean LCOE (USD/MWh) for tech, fallback on miss."""
        if self._raw is None:
            return fallback
        try:
            # Pydantic LCOEResult path
            if hasattr(self._raw, "techs"):
                tech_obj = self._raw.techs.get(tech)
                if tech_obj is None:
                    return fallback
                # LCOEResult.techs[tech].stats.mean  or  .lcoe_usd_mwh
                stats = getattr(tech_obj, "stats", None)
                if stats is not None:
                    val = getattr(stats, "mean", None)
                    if val is not None:
                        return float(val)
                return fallback

            # Serialised dict path (from orchestrator cache)
            if isinstance(self._raw, dict):
    ...
    def normalised_score(self, tech: str) -> Optional[float]:
        """
        Normalised LCOE score [0–1] where 1 = cheapest technology.
        Returns None when only one technology is available (no range).
        """
        all_lcoes = {
            t: self.mean_lcoe(t) for t in ("solar", "wind", "biomass")
        }
        vals = list(all_lcoes.values())
        lo, hi = min(vals), max(vals)
        if hi <= lo:
            return None
        v = all_lcoes.get(tech, hi)
        return (hi - v) / (hi - lo)   # higher score = lower LCOE
...
class TransportDecarbonizationCalculator:
    """
    Phase 9: Annual transport fleet electrification modelling (2025–2050).
    ...
    def __init__(
        ...
        # ── Locate transport_parameters.json ─────────────────────────────
    ...
    def run(
        ...
        context_gdf,
        country_params,
        suitability_threshold: float = 0.3,   # ← new parameter with safe default
    ) -> Dict[str, Any]:
        """
        Execute Phase 9: transport electrification projections.

        Args:
            country_code:    ISO-3166 alpha-3 code.
            country_name:    Human-readable country name.
            mainland_gdf:    Country boundary GeoDataFrame.
            country_params:  Typed CountryParams schema (optional; Phase 9
                             reads most parameters from transport_parameters.json,
                             but country_params is used for logging traceability
                             and future CF/threshold consistency checks).
            pot_results:     Phase 4 output (PotentialResult or dict or None).
            lcoe_results:    Phase 5 output (LCOEResult or dict or None).
            suitability_dir: Path to OWA suitability TIFs from Phase 3.
            context_gdf:     Neighbouring countries for cartographic context.

        Returns:
            Dict with keys: timeseries_df, fleet_df, hubs_gdf, summary,
            report_path.
        """
        t_start  = time.perf_counter()
        code     = country_code

        # ── Wrap upstream results in typed views ──────────────────────────
        pot_view  = _PotentialView(pot_results)
        lcoe_view = _LCOEView(lcoe_results)

        # ── Log CountryParams traceability ────────────────────────────────
        if country_params is not None:
            logger.info(
                "  [CountryParams] %s — solar CF=%.3f | wind CF=%.3f | "
                "biomass CF=%.3f | threshold=%.2f",
                country_params.country_code,
                country_params.solar_capacity_factor,
                country_params.wind_capacity_factor,
                country_params.biomass_capacity_factor,
                suitability_threshold,
            )
        else:
            logger.info(
                "  [CountryParams] Not provided — "
                "transport_parameters.json is the sole parameter source."
            )

        # ── Validate suitability dir ──────────────────────────────────────
        if suitability_dir is not None:
            suitability_dir = Path(suitability_dir)
            if suitability_dir.exists():
                tifs = sorted(suitability_dir.glob("*.tif"))
                logger.info("  Input suitability: %d TIF files found", len(tifs))
                for tif in tifs[:3]:
                    logger.info("    - %s", tif.name)
            else:
                logger.warning(
                    "  Suitability directory not found: %s", suitability_dir
                )

        # ── Log Phase 4 potential ─────────────────────────────────────────
        if pot_view.available():
            for tech in TECH_ORDER:
                gw = pot_view.capacity_gw(tech, "balanced")
                if gw > 0:
                    logger.info("  Input %-12s: %.2f GW (balanced)", tech, gw)
        else:
            logger.info("  Phase 4 results not available — RE potential = 0.")

        # ── Log Phase 5 LCOE ─────────────────────────────────────────────
        if lcoe_view.available():
            for tech in TECH_ORDER:
                lcoe = lcoe_view.mean_lcoe(tech)
                logger.info("  Input LCOE %-9s: %.1f USD/MWh", tech, lcoe)
        else:
            logger.info("  Phase 5 results not available — using LCOE fallbacks.")
        ...
        for d in (out_data, out_rep, out_fig):
        ...
        self._log_parameter_dashboard(
            code, country_name, country_tp, defaults, years, country_params
        )

        # ── Steps 1–6 ────────────────────────────────────────────────────
        logger.info("  [1/7] Building fleet size trajectory ...")
        fleet_df = self._build_fleet_trajectory(
            code, country_tp, defaults, years
        )

        logger.info("  [2/7] Calculating EV electricity demand & RE requirements ...")
        ts_df = self._build_timeseries(
            code, country_tp, defaults, fleet_df, years, pot_view
        )

        logger.info("  [3/7] Computing GHG emission trajectories ...")
        ts_df = self._compute_emissions(
            ts_df, fleet_df, country_tp, defaults, years
        )

        logger.info("  [4/7] Computing transport electrification costs ...")
        ts_df = self._compute_costs(
            ts_df, country_tp, defaults, years, lcoe_view
        )

        logger.info("  [5/7] Locating charging hubs from suitability rasters ...")
        hubs_gdf = self._place_charging_hubs(
            code, country_tp, defaults, mainland_gdf,
            suitability_dir, lcoe_view,
        )

        logger.info("  [6/7] Building summary ...")
        summary = self._build_summary(
            code, country_name, ts_df, fleet_df, hubs_gdf, pot_view
        )
        ...
        plot_emissions_trajectory(
            ...
            out_fig / f"{code}_transport_emissions_trajectory.png",
        )
        plot_fleet_transition(
            self.styler,
            fleet_df, country_name, code,
            out_fig / f"{code}_transport_fleet_transition.png",
        )
        plot_renewable_need(
            self.styler,
            ts_df, country_name, code,
            out_fig / f"{code}_transport_renewable_need.png",
        )
        if hubs_gdf is not None and not hubs_gdf.empty:
            plot_hub_map(
                self.styler, self._admin_gdf,
                hubs_gdf, mainland_gdf, context_gdf, country_name,
                code, suitability_dir,
                out_fig / f"{code}_transport_hubs_2050.png",
            )

        # ── Verify outputs ────────────────────────────────────────────────
        expected_plots = [
            f"{code}_transport_emissions_trajectory.png",
            f"{code}_transport_fleet_transition.png",
            f"{code}_transport_renewable_need.png",
            f"{code}_transport_hubs_2050.png",
        ]
        missing_plots = [
            p for p in expected_plots if not (out_fig / p).exists()
        ]
        if missing_plots:
            logger.warning("  Missing plots: %s", ", ".join(missing_plots))
        else:
            logger.info("  All expected transport plots generated successfully.")

        # ── Persist artefacts ─────────────────────────────────────────────
        elapsed = time.perf_counter() - t_start
        logger.info(
            "Phase 9 (Transport Decarbonisation) completed in %.1fs", elapsed
        )

        from src.io.artifact_manager import ArtifactManager
        artifact_mgr = ArtifactManager(self.outputs_dir, country_code)
        phase_dir    = artifact_mgr.phase_dir("transport")
        ...
        serializable_result = {
            ...
            "summary":       summary,
            "n_hubs":        len(hubs_gdf) if hubs_gdf is not None else 0,
            "elapsed":       elapsed,
        }
        artifact_mgr.save_result(
            phase_dir, serializable_result, serializer="pickle"
        )
    ...
    # ─────────────────────────────────────────────────────────────────────
    # Step 1: Fleet trajectory
    # ─────────────────────────────────────────────────────────────────────

    def _build_fleet_trajectory(
        self,
        code:       str,
        country_tp: Dict[str, Any],
        ...
        # ── Resolve base light-vehicle fleet (3-tier) ────────────────────
        ...
        if lv_key in country_tp:
            ...
            pc_share    = float(
                vehicle_cats["passenger_car"].get("share_of_fleet", 0.72)
            )
        ...
        for sc in ("reference", "accelerated", "conservative"):
            raw_sc = (
                pt_override.get(sc)
                or defaults["powertrain_penetration_scenarios"].get(sc, {})
            )
        ...
        for sc in ("reference", "accelerated", "conservative"):
            ...
            stocks            = {
                pt: base_lv_fleet * init_shares.get(pt, 0.0)
                for pt in _POWERTRAIN_TYPES
            }
            ...
            for y in years:
                ...
                records.append({
                    ...
                    "bev_share_pct":         round(
                        100 * bev_stock / max(fleet_size, 1), 2
                    ),
                    "electrified_share_pct": round(
                        100 * electrified / max(fleet_size, 1), 2
                    ),
    ...
    def _build_timeseries(
        ...
        years:       List[int],
        pot_view:    _PotentialView,    # ✅ typed view instead of raw dict
    ) -> pd.DataFrame:
        vehicle_cats = defaults["vehicle_categories"]

        # ✅ Use typed view — no inline dict access
        re_avail = pot_view.as_re_dict()
        ...
        for sc in ("reference", "accelerated", "conservative"):
            for y in years:
                total_ev_twh = 0.0
                total_fleet  = 0.0
                total_bev = total_hev = total_phev = total_ice = total_fcev = 0.0
                ...
                gw_solar = (
                    total_ev_twh * solar_share * 1000 / (cf_solar * 8760)
                    if total_ev_twh > 0 else 0.0
                )
                gw_wind = (
                    total_ev_twh * wind_share * 1000 / (cf_wind * 8760)
                    if total_ev_twh > 0 else 0.0
                )
                ...
                delta_solar = max(
                    0.0,
                    gw_solar - (prev["re_cumulative_solar_gw"] if prev else 0.0),
                )
                delta_wind = max(
                    0.0,
                    gw_wind - (prev["re_cumulative_wind_gw"] if prev else 0.0),
                )
    ...
    # ─────────────────────────────────────────────────────────────────────
    # Step 3: GHG trajectories  (unchanged logic — only signature updated)
    # ─────────────────────────────────────────────────────────────────────
    ...
    def _compute_emissions(
        ...
        def _weighted_ice_ef(cat: str) -> float:
            ...
            non_ev  = {
                k: v for k, v in cat_mix.items()
                if k not in (
                    "bev", "fcev", "hydrogen_fcev", "hybrid_hev", "hybrid_phev"
                )
            }
        ...
        # Baseline: full-ICE counterfactual
        baseline_by_year: Dict[int, float] = {}
        for y in years:
            ref_row = ts_df[
                (ts_df.year == y) & (ts_df.scenario == "reference")
            ]
            if ref_row.empty:
                baseline_by_year[y] = 0.0
                continue
            total_fleet = float(
                ref_row["total_fleet_all_cats"].values[0]
            )
            co2 = 0.0
            for cat, cat_cfg in vehicle_cats.items():
        ...
        for sc in ("reference", "accelerated", "conservative"):
            for y in years:
                ...
                co2_ice = co2_hev = co2_phev = co2_bev = 0.0
    ...
    def _compute_costs(
        ...
        defaults:   Dict[str, Any],
        years:      List[int],
        lcoe_view:  _LCOEView,          # ✅ typed view instead of raw dict
    ) -> pd.DataFrame:
        """
        Cost model with LCOE integration.

        When lcoe_view has data (Phase 5 available):
          Energy cost = EV_demand_TWh × source_share × LCOE_USD_MWh × 1000 (→ bn USD)
        Otherwise falls back to fixed capex from transport_parameters.json.
        """
        ...
        # ✅ Use typed view — no inline dict access
        using_lcoe           = lcoe_view.available()
        lcoe_solar_usd_mwh   = lcoe_view.mean_lcoe("solar",   fallback=55.0)
        lcoe_wind_usd_mwh    = lcoe_view.mean_lcoe("wind",    fallback=48.0)
        lcoe_biomass_usd_mwh = lcoe_view.mean_lcoe("biomass", fallback=85.0)

        if using_lcoe:
            logger.info(
                "    Using Phase-5 LCOEs — "
                "Solar: %.1f | Wind: %.1f | Biomass: %.1f USD/MWh",
                lcoe_solar_usd_mwh, lcoe_wind_usd_mwh, lcoe_biomass_usd_mwh,
            )
        else:
            logger.info(
                "    LCOE results not available — "
                "using JSON capex defaults (solar: %.0f | wind: %.0f USD/kW)",
        ...
        for sc in ("reference", "accelerated", "conservative"):
            ...
            for y in years:
                ...
                if using_lcoe:
                    energy_cost_solar = (
                        ev_twh * solar_share * 1e6 * lcoe_solar_usd_mwh / 1e9
                    )
                    energy_cost_wind  = (
                        ev_twh * wind_share * 1e6 * lcoe_wind_usd_mwh / 1e9
                    )
                ...
                fuel_savings = (
                    ev_twh * 1e9 * (fuel_usd_per_kwh - elec_usd_per_kwh)
                ) / 1e9
                ...
                cost_rows.append({
                    ...
                    "lcoe_solar_usd_mwh": (
                        round(lcoe_solar_usd_mwh, 1) if using_lcoe else None
                    ),
                    "lcoe_wind_usd_mwh": (
                        round(lcoe_wind_usd_mwh, 1)  if using_lcoe else None
                    ),
    ...
    def _place_charging_hubs(
        ...
        """
        Hub placement with composite score:
          score = w_suit × suitability + w_dist × proximity + w_lcoe × lcoe_score

        Uses _LCOEView.normalised_score() instead of inline dict access.
        """
        ...
        if suitability_dir is None:
            logger.warning(
                "No suitability_dir provided — hub placement skipped."
            )
            return None
        ...
        tif_solar = sorted(
            suitability_dir.glob(f"{code}_solar_suitability*.tif")
        )
        tif_wind  = sorted(
            suitability_dir.glob(f"{code}_wind_suitability*.tif")
        )
        ...
        if not tif_solar and not tif_wind:
            ...
            if not fallback:
                logger.warning(
                    "No suitability TIFFs found — hub placement skipped."
                )
                return None
        ...
        try:
            land_union = (
                mainland_gdf.union_all()
                if hasattr(mainland_gdf, "union_all")
                else mainland_gdf.unary_union
            )
            land_prep      = prep(land_union)
            bounds         = land_union.bounds
            ...
            logger.warning(
                "Could not build land mask: %s — skipping spatial filter", exc
            )
            land_prep = centroid = max_remote_deg = None
            bounds    = mainland_gdf.total_bounds

        # ── Demand grid ───────────────────────────────────────────────────
        minx_b = float(bounds[0])
        miny_b = float(bounds[1])
        maxx_b = float(bounds[2])
        maxy_b = float(bounds[3])
        ...
        # ✅ LCOE score via typed view — no inline dict access
        lcoe_by_source: Dict[str, float] = {}
        if lcoe_view.available():
            for src_label, tech_key in [("Solar", "solar"), ("Wind", "wind")]:
                score = lcoe_view.normalised_score(tech_key)
                if score is not None:
                    lcoe_by_source[src_label] = score
        ...
        if tif_solar:
            all_sources.append(("Solar", tif_solar[0]))
        if tif_wind:
            all_sources.append(("Wind",  tif_wind[0]))
        ...
        logger.info(
            "    Score weights — suitability: %.2f | proximity: %.2f | LCOE: %.2f",
        ...
        for source_label, tif_path in all_sources:
            try:
                ...
                logger.error(
                    "Failed to read %s raster: %s", source_label, exc
                )
                continue
            ...
            src_lcoe_score = lcoe_by_source.get(source_label, 0.5)

            for lon_p, lat_p, suit_p in zip(lons_raw, lats_raw, scores_raw):

                if land_prep is not None:
                    if not land_prep.contains(Point(lon_p, lat_p)):
                        continue
                ...
                if centroid is not None and max_remote_deg is not None:
                    ...
                    if dist > max_remote_deg:
                        continue
                    proximity_score = 1.0 - (dist / max_remote_deg)
                else:
                    proximity_score = 0.5
                ...
                all_candidates.append(
                    (composite, lon_p, lat_p, source_label)
                )

        if not all_candidates:
            logger.warning(
                "Hub placement: no candidates passed geographic filters."
            )
            return None

        all_candidates.sort(key=lambda x: -x[0])

        # ── Greedy placement with demand-grid quota ───────────────────────
        n_cands           = len(all_candidates)
        max_hubs_per_cell = max(
            1, int(np.ceil(np.sqrt(n_cands) / demand_grid_n))
        )
        ...
        hub_df = pd.DataFrame({
            ...
            "hub_id":             [
                f"{code}_HUB_{i + 1:04d}" for i in range(len(acc_lons))
            ],
        ...
        geometry = [_Pt(lo, la) for lo, la in zip(acc_lons, acc_lats)]
    ...
    def _build_summary(
        ...
        pot_view:     _PotentialView,   # ✅ typed view
    ) -> Dict[str, Any]:
        re_avail = pot_view.as_re_dict()
        ...
        for sc in ("reference", "accelerated", "conservative"):
            ...
            re_needed_gw  = float(last.get("re_needed_total_gw", 999))
            re_total_avail = re_avail.get("solar", 0) + re_avail.get("wind", 0)
            ...
            summary["scenarios"][sc] = {
                ...
                "ev_demand_vs_re_potential": (
                    "COVERED"     if re_needed_gw <= re_total_avail else
                    "PARTIAL"     if re_total_avail > re_needed_gw * 0.5 else
                    "INSUFFICIENT"
                ),
            }
    ...
    def _log_parameter_dashboard(
        ...
        # Log RE parameters from CountryParams for traceability
        if country_params is not None:
            logger.info(
                "  [RE params from CountryParams] "
                "solar CF=%.3f | wind CF=%.3f",
                country_params.solar_capacity_factor,
                country_params.wind_capacity_factor,
            )
        ...
        sc_data = (
            pt_override.get("reference", {})
            or defaults["powertrain_penetration_scenarios"].get("reference", {})
        )
    ...
    # ─────────────────────────────────────────────────────────────────────
    # Text report  (unchanged — only helper removed)
    # ─────────────────────────────────────────────────────────────────────
    ...
    def _format_report(
        ...
        # Annual timeseries, scenario comparison, fleet composition,
        # charging infrastructure, methodology — all unchanged
        lines += [
            "",
            sep,
        ...
        for y in [
            yr for yr in (2025, 2030, 2035, 2040, 2045, 2050)
        ...
        lines += [
            ...
            f"  Grid demand    : BEV full + PHEV partial (utility factor "
